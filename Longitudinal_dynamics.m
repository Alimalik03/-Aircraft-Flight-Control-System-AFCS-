% Longitudinal Dynamics Simulation Script
...
% Extract longitudinal states: theta, u, w, q
long_states = [5, 7, 9, 14];
A_long = A(long_states, long_states);
B_long = B(long_states, 1); % Elevator only
C_long = eye(4);
D_long = zeros(4,1);
sys_long = ss(A_long, B_long, C_long, D_long);

%% Step 4: Simulate Step Response to Elevator Input
t = 0:0.1:60;
delta_e_input = deg2rad(1); % 2 deg step
u = delta_e_input * ones(size(t));
[y, t_out, x] = lsim(sys_long, u, t);

%% Step 5: Plot Longitudinal States
labels = {'Pitch Angle \theta (deg)', 'Axial Velocity u (m/s)', ...
          'Vertical Velocity w (m/s)', 'Pitch Rate q (deg/s)'};
